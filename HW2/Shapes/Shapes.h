...
public:
    comboShape(const vector<Shape *> &s):shapes(s){}
    double area()const
    {
...
//find shapes largest area
double maxArea(const vector<Shape *>shapes)
{
    double maxArea=0;
    for(Shape *p:shapes)
    {
...
//for using sort algorithm to decreasing value
bool sortfun(Shape *p1,Shape *p2)
{
...
{
    sort(shapes.begin(),shapes.end(),sortfun);
}
